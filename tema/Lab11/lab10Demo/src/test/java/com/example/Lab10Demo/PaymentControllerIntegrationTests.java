...
public class PaymentControllerIntegrationTests {
    ...
    public static void setup() {
        sender = new UserModel(id(), "John", "Doe", SENDER_EMAIL, "password");
    ...
    public void testSaveAccounts() throws Exception {
        ...
        when(repository.getUserAccounts(sender.getId())).thenReturn(receiverAccounts);
        mockMvc.perform(MockMvcRequestBuilders.put(endpoint)
                .header((Credentials.AUTHORIZATION), credentials(adminEmail, adminPassword))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(senderAccounts.get(0))))
    ...
    public void testDeleteAccounts() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.delete(endpoint)
                .header((Credentials.AUTHORIZATION), credentials(sender))
                .queryParam("email", SENDER_EMAIL)
                .queryParam("currency", AccountModel.Currency.USD.name()))
                .andExpect(MockMvcResultMatchers.status().isForbidden());
        ...
        when(repository.getUserAccounts(sender.getId())).thenReturn(senderAccounts);
        when(repository.removeAccount(any(), any(AccountModel.Currency.class))).thenReturn(true);
        mockMvc.perform(MockMvcRequestBuilders.delete(endpoint)
                .header((Credentials.AUTHORIZATION), credentials(adminEmail, adminPassword))
                .queryParam("email", adminEmail)
                .queryParam("currency", AccountModel.Currency.USD.name()))
